...
直接运行：
  python backend/run_models/run_reranking.py
会根据 `Config.RERANK_SERVE` 中的 `model_path`, `served_name`, `port`, `gpu_memory_utilization` 构建命令并执行。
"""
...
def build_command(cfg: dict) -> str:
    ...
    if not model_path or not port:
        raise SystemExit("RERANK_SERVE 配置缺少 model_path 或 port")
...
def main():
    cfg = getattr(Config, "RERANK_SERVE", None)
    if not cfg:
        raise SystemExit("未在 Config 中找到 RERANK_SERVE 配置")
